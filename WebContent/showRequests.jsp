					...
					<c:when test="${request.traineeRequest eq true}">
						...
						<tr id="${table.index}">
							<td class="span6">${request.connection.trainee.firstName}
								${request.connection.trainee.lastName} wants to be your trainee
								in ${request.connection.field.title}</td>
							<td class="span4">
								<button class="btn btn-success"
									onclick="answerYes('${table.index}','${request.requestID}')">Accept</button>
								<button class="btn btn-danger"
									onclick="answerNo('${table.index}','${request.requestID}')">Deny</button>
