...
<div class="container well" style="box-shadow: 5px 5px 8px -1px #222;">
	...
	<c:choose>
		<c:when test="${nullUser eq true}">
			<div class="alert alert-error">Given user does not exist or the
				given argument is invalid. Please try again.</div>
		</c:when>
		<c:otherwise>
			<div class="row-fluid">
				<div class="span12">
			...
			<div class="row-fluid">
				<div class="span6">
					...
					<div class="row-fluid">
						...
						<div class="span8">${profile.locationCity},
							${profile.locationCountry}</div>
					</div>
					<div class="row-fluid">
						<div class="span12">
							<a class="btn btn-primary" type="submit"
								href="mailto:${profile.email}">Contact</a>
						</div>
					</div>
				</div>
				<div class="span6">
					<img
						src="http://maps.googleapis.com/maps/api/staticmap?center=${profile.locationCity},${profile.locationCountry}&zoom=12&size=500x350&sensor=false">
				</div>
			</div>
		</c:otherwise>
	</c:choose>
